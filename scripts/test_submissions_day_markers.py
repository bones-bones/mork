...
import hc_constants
from cogs.lifecycle.submissions_day_markers import (
    day_marker_content,
    day_marker_iso_date,
...
def test_is_and_parse_marker():
    m = _msg(day_marker_content(datetime(2026, 6, 1, tzinfo=UTC)))
...
def test_previous_week_body():
    markers = [
        _msg(day_marker_content(datetime(2026, 5, 31, tzinfo=UTC))),
        _msg(day_marker_content(datetime(2026, 5, 30, tzinfo=UTC))),
    ]
...
def test_is_submissions_card():
    ...
    assert not is_submissions_card(
        _msg(day_marker_content(datetime(2026, 6, 1, tzinfo=UTC)))
    )
    assert not is_submissions_card(_msg("no attachment", author_id=hc_constants.MORK_2))
    assert not is_submissions_card(
        _msg(
            "reminder",
            author_id=hc_constants.MORK_2,
            attachments=[_attachment("notes.txt", "text/plain")],
        )
    )
...
def test_has_image_attachment():
    ...
    assert not has_image_attachment(
        _msg("file", attachments=[_attachment("doc.pdf", "application/pdf")])
    )


def test_submission_period_start():
    now = datetime(2026, 6, 3, 1, 0, tzinfo=UTC)
    marker = _msg(day_marker_content(datetime(2026, 6, 1, 1, 0, tzinfo=UTC)))
    marker.created_at = datetime(2026, 6, 1, 1, 0, tzinfo=UTC)  # type: ignore
    assert submission_period_start(now, None) == utc_day_start(now) - timedelta(days=1)
    assert submission_period_start(now, marker) == datetime(2026, 6, 2, 0, 0, tzinfo=UTC)
