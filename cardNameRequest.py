...
def cardNameRequest(requestName: str):
    ...
    for cardName in allCards.keys():
        ...
        if currentWeight > maxWeight:
            ...
            maxWeightName = cardName
        elif currentWeight == maxWeight and (
            len(cardName) < len(maxWeightName)
        ):  # if the similarity is a tie, use the shorter name. seems okay?
            maxWeightName = cardName
    ...
    for i in range(len(requestName)):
        for j in range(len(name)):
            ...
            for x in range(1, minOfNameAndRequest + 1):
                if (
                    requestName[i : i + x] != name[j : j + x]
                    or x == minOfNameAndRequest
                ):
                    weight += max(
                        (x - 1) * (x - 1) - 1, 0
                    )  # the further along in the matching we are, the better
                    break
    return weight
